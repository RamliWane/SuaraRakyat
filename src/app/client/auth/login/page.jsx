...
export default function Login() {
    ...
    const handleChange = (e) => {
        ...
        // spread form lama, lalu update field yang berubah berdasarkan name attribute input
    };

    const handleSubmit = async (e) => {
        e.preventDefault();  // mencegah page reload saat form disubmit
        ...
        setLoading(true);     // aktifin loading state

        try {
            const data = await api.post('/auth/login', {  // kirim request POST ke backend
                email: form.email,
                password: form.password
            });

            localStorage.setItem('token', data.token);  // simpen token JWT di browser

            router.push('/client/homepage');  // redirect ke halaman client setelah login berhasil
        } catch (err) {
            setError(err.message);  // tampilkan pesan error dari backend
        } finally {
            setLoading(false);  // matiin loading state apapun hasilnya
        }
    };
        ...
        <div className="flex h-screen items-center justify-center">
            <div className="w-full md:w-2/5 bg-white flex items-center justify-center p-8">
                <div className="w-full max-w-md">
                    <h1 className="text-4xl text-black   font-bold mb-2">SIGN IN</h1>
                    <p className="text-gray-600 mb-8" style={{ fontFamily: "'Hanken Grotesk', sans-serif" }}>
                        ...
                        <div className="flex justify-between items-center text-sm">
                            <span className="text-gray-700">Have Trouble In Sign In?</span>
                            <a
